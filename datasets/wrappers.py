...
class ValDataset(Dataset):
    ...
    def __getitem__(self, idx):
        img, mask, bmask = self.dataset[idx]

        return {
            'inp': self.img_transform(img),
            'gt': self.mask_transform(mask),
            'bgt':self.mask_transform(bmask)
        }
...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if random.random() < 0.5:
            ...
            bmask = bmask.transpose(Image.FLIP_LEFT_RIGHT)
        ...
        bmask = transforms.Resize((self.inp_size, self.inp_size), interpolation=InterpolationMode.NEAREST)(bmask)
